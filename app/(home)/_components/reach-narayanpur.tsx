...
export const ReachNarayanpur = () => {
  ...
  const filteredRoutes =
    activeTab.toLowerCase() === "all"
      ? busRoutes
      : busRoutes.filter(
          (route) => route.from.toLowerCase() === activeTab.toLowerCase(),
        );
    ...
    >
      ...
      <motion.div layout className="grid gap-6 md:grid-cols-2">
        {filteredRoutes.map((route, index) => (
          ...
          >
            {/* Card Header */}
            <div className="bg-primary-light px-4 py-3 border-b border-gray-100">
                  ...
                  {route.timings.sort().map((time, timeIndex) => (
                    <motion.span
                      ...
                      className="inline-flex items-center px-3 py-1 rounded-full text-sm
                        bg-[#FFFFFF] text-primary font-medium border-b-2 border-t-cyan-100"
                    >
                      {time}
                    </motion.span>
                  ))}
      ...
      {filteredRoutes.length === 0 && (
        ...
        >
          <p className="text-gray-500 text-lg">
            No bus services found for this route.
          </p>
        </motion.div>
      )}
    </motion.div>
